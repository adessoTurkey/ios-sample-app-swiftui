...
class WebSocketStream: NSObject, AsyncSequence {
    ...
    init(url: URL, session: URLSession = URLSession.shared, loggerManager: LoggerManagerProtocol?) {
    ...
    func schedulePing() {
        ...
        pingTimer = Timer.scheduledTimer(withTimeInterval: pingTimeInterval, repeats: true) { [weak self] timer in
            ...
            self.task.sendPing { error in
                if let error {
                    self.loggerManager?.setError("Pong Error: \(error.localizedDescription)")
                    timer.invalidate()
                } else {
                    self.loggerManager?.setInfo("Connection is alive")
                }
            }
        }
    }
