...
protocol AllCoinUseCaseProtocol {
    ...
    func fetchTopVolume(limit: Int, unitToBeConverted: String, page: Int) async throws -> AllCoinUIModel
}
...
class AllCoinUseCase: AllCoinUseCaseProtocol {
    ...
    func fetchTopVolume(limit: Int, unitToBeConverted: String, page: Int) async throws -> AllCoinUIModel {
        let responseModel = try await allCoinRepository.getTopVolume(limit: limit, unitToBeConverted: unitToBeConverted, page: page)
        return AllCoinUIModel(from: responseModel)
    }
}
