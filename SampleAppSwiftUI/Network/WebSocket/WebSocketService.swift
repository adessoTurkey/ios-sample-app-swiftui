...
protocol WebSocketServiceProtocol: AnyObject, Publisher where Self.Output == URLSessionWebSocketTask.Message, Self.Failure == Error {
    ...
    func disconnect()
    func connectionHandler(connected: @escaping (any WebSocketServiceProtocol) -> (),
                           disconnected:  @escaping (URLSessionWebSocketTask.CloseCode) -> ()) -> AnyPublisher<URLSessionWebSocketTask.Message,Error>
...
final class WebSocketService: NSObject, WebSocketServiceProtocol  {
    ...
    private var disconnectionHandler: (URLSessionWebSocketTask.CloseCode) -> () = {_ in}
    private var connectionHandler: (any WebSocketServiceProtocol) -> () = {_ in}
