...
//
//  Created by Yildirim, Alper on 21.10.2023.
//
...
class CoinPriceHistoryChartViewModel: ObservableObject {
    ...
    func fetchCoinPriceHistory(for coin: CoinData) async {
        pageState = .loading
        ...
        Task {
            let response = try? await coinPriceHistoryUseCase.getHourlyPriceHistory(
                coinCode: coinCode,
